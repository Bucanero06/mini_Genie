...
def simulate_suggestions(self):
    ...
    for epoch_n, epoch_params_record_id in enumerate(chunks_ids_of_params_left_to_compute):
        #
        start_time = perf_counter()
        gc.collect()
        CHECKTEMPS(TEMP_DICT)
        #
        if epoch_n not in (0, stop_after_n_epoch, n_chunks):
            '''Run Epochs n=1 to n=n_chunks  
                    1.  Simulate Signals (n) & Analyze (n-1) & Save (n-1)
                    2.  Simulate Events  (n)
            '''
            func_calls = []
            #
            func_calls.append(
                simulation_handler.simulate_signals.remote(simulation_handler_id, epoch_params_record_id,
                                                           self.runtime_settings[
                                                               "RAY_SETTINGS.simulate_signals_num_cpus"]))
            #
            # func_calls.append(
            #     analysis_handler.compute_stats.remote(analysis_handler_id, pf, self.metrics_key_names))
            only_these_stats = np.array_split(self.metrics_key_names, 2)
            func_calls.append(
                analysis_handler.compute_stats.remote(analysis_handler_id, pf, only_these_stats[0]))
            func_calls.append(
                analysis_handler.compute_stats.remote(analysis_handler_id, pf, only_these_stats[1]))
            #
            simulation_result, portfolio_stats_1, portfolio_stats_2 = ray.get(func_calls)
            #
            long_entries, long_exits, short_entries, short_exits, strategy_specific_kwargs = simulation_result
            portfolio_stats = portfolio_stats_1.join(portfolio_stats_2)
            #
            highest_profit, best_parameters = _analyze_n_save(portfolio_stats, previous_epoch_params_record_id,
            ...
            '''Run Initial Epoch 
                    1.  Simulate Signals (n)
                    2.  Simulate Events (n)
            '''
            long_entries, long_exits, short_entries, short_exits, \
            strategy_specific_kwargs = ray.get(
                simulation_handler.simulate_signals.remote(simulation_handler_id, epoch_params_record_id,
                                                           self.runtime_settings[
                                                               "RAY_SETTINGS.simulate_signals_num_cpus"]
                                                           ))
            ...
            pf, extra_sim_info = simulation_handler.simulate_events(long_entries, long_exits,
                                                                    ...
                                                                    strategy_specific_kwargs)
            #
            # atexit.register(previous_epoch_params_record_id, highest_profit, best_parameters,
            #                 initial_cash_total, epoch_n, save_every_nth_chunk=None)
            #
        #
        if epoch_n in (stop_after_n_epoch, n_chunks):
            '''Run Finishing Epoch
                    1.  Analyze (n-1)
                    2.  Save    (n-1)
            '''
            _analyze_n_save(portfolio_stats, epoch_params_record_id, highest_profit, best_parameters,
                            initial_cash_total, epoch_n,
                            save_every_nth_chunk=1)
            #
            # todo: some comment here for the finish :)
            break

        previous_epoch_params_record_id = epoch_params_record_id
